...
```
## d
The p-value is smaller than 5% and the 95 percent confidence interval for IRR is (1.02, 1.04). The predicted tympanometric width differs significantly between men and women.
```{r}
...
# Problem 2
## a
I queried chatGPT for apply functions to write active ratios in SQL
```{r}
library(DBI)
...
base_method <- function() {
  ...
  tbl <- data.frame(
    store_id = c(1, 2),
    active_ratio = c(
      sum(store1$active == 1) / nrow(store1) * 100,
      sum(store2$active == 1) / nrow(store2) * 100
    ),
...
sql_method <- function() {
  dbGetQuery(
    sakila,
    "SELECT store_id,
            100.0 * AVG(active) AS active_ratio,
...
base_method <- function() {
  ...
  result <- result[, c("staff_id", "first_name", "last_name", "country_id", "country")]
  result
}
...
sql_method <- function() {
 dbGetQuery(
   sakila,
   "SELECT staff.staff_id AS staff_id, staff.first_name AS first_name, staff.last_name AS last_name, add_country.country_id AS country_id, add_country.country AS country 
    FROM staff 
    LEFT JOIN
      (SELECT add_city.addr_id AS addr_id, country.country AS country, country.country_id AS country_id
...
```
### c
I queried chatGPT for the base R expressions and then modified the codes.
```{r}
# Base R method
#' Regular R expression version 
base_method <- function() {
  ...
  merged <- merge(payment, rental, by = "rental_id", all.x = TRUE)
  merged <- merge(merged, inventory, by = "inventory_id", all.x = TRUE)
  merged <- merge(merged, film[, c("film_id", "title")], by = "film_id", all.x = TRUE)

  result <- aggregate(merged$amount, by = list(film_title = merged$title), FUN = sum)

  colnames(result)[2] <- "total_amount"
  result <- result[order(result$total_amount, decreasing = TRUE), ]
...
## a
```{r}
records <- read.csv("au-500.csv")
grep("\\.com$", records$web)/length(records$web)
```

## b
```{r}
emails <- strsplit(records$email, "@")
domain_name <- sapply(emails, "[[", 2)
tbl <- table(domain_name)
most_common <- names(tbl)[which.max(tbl)]
most_common
...
```{r}
# Including &
mean(grepl("[^a-zA-Z, ]", records$company_name))
# Excluding &
mean(grepl("[^a-zA-Z, &]", records$company_name))
...
## d
```{r}
phone1_part1 <- substr(records$phone1, 1, 2)
phone1_part2 <- substr(records$phone1, 4, 7)
phone1_part3 <- substr(records$phone1, 9, 12)

phone1 <- paste(phone1_part1, phone1_part2, phone1_part3, sep = "")
phone1_part1 <- substr(phone1 , 1, 4)
phone1_part2 <- substr(phone1 , 5, 7)
phone1_part3 <- substr(phone1 , 8, 10)

phone1 <- paste(phone1_part1, phone1_part2, phone1_part3, sep = "-")
records$phone1 <-phone1
records$phone1[1:10]
records$phone2[1:10]
```
...
## f
The apartment numbers do not follow Benford’s Law, as the distribution of the leading digits is not decreasing as expected.
```{r}
